...
### Snapshot Storage and Growth Behavior

SQL Server uses a **copy-on-write** mechanism for database snapshots, which has important implications for storage:

#### How Snapshot Storage Works

1. **Initial Creation**: When you create a snapshot, it starts as a **sparse file** that takes minimal space initially
2. **Growth on Changes**: Each time a data page in the source database is modified **after** a snapshot was created, SQL Server:
   - Copies the original (unchanged) page to the snapshot's sparse file
   - Allows the modification to proceed on the source database
   - The snapshot file grows by the size of that page

#### Multiple Snapshots and Storage Growth

**Each snapshot grows independently** based on changes made to the source database **after its creation**:

**Example Scenario**: You have snapshots Automatic, Alpha, and Bravo (created in that order)

- **Automatic snapshot**: Grows with **ALL changes** made to the source database
- **Alpha snapshot**: Grows with changes made **after Alpha was created**
- **Bravo snapshot**: Grows with changes made **after Bravo was created**

#### Storage Implications

- **Maximum Size**: Each snapshot can potentially grow to the **full size of the original database** if enough changes occur
- **Independent Growth**: The more changes you make to the source database, the larger all snapshots become
- **Storage Monitoring**: Monitor disk space usage because snapshots can consume significant storage
- **File Management**: SQLParrot tracks physical file names for each snapshot (`.ss` files) and can verify their existence

#### Why This Matters

- **Storage Planning**: Plan for snapshot storage requirements based on expected database changes
- **Cleanup Strategy**: Use SQLParrot's cleanup functionality to manage storage usage
- **Restore Benefits**: When you restore from a snapshot, SQL Server automatically removes that snapshot, helping manage storage
...
// 3. Remove all snapshots from SQL Server metadata
await removeSnapshotsFromMetadata(targetGroupId);
...
6. Consider resetting the application state by dropping the `sqlparrot` database
...
4. Verify the SQL Server metadata is in sync with actual snapshots
