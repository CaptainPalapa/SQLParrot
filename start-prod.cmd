...
if not exist ".env" (
    ...
    echo You can run setup-env.cmd to help with this.
    echo.
    pause
    exit /b 1
)
...
if %errorlevel% equ 0 (
    ...
    echo Please stop the conflicting application or run stop-dev.cmd
    pause
    exit /b 1
)
...
echo.
echo       or run stop-dev.cmd to force stop all processes.
echo.
